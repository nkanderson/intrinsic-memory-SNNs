// Nexys A7-100T board top for the bitshift-custom_slow_decay SNN configuration.
...
//
// Pins (see constraints/nexys_a7_bitshift_custom_slow_decay.xdc):
...
// `cp_integration_bitshift_custom_slow_decay` so the FPGA matches the simulated reference.

module board_top_bitshift_custom_slow_decay (
    ...
    // CPU_RESETN is active-low; invert to active-high for the internal logic.
    wire reset_int;
    assign reset_int = ~CPU_RESETN;

    // --- Accelerator wrapper ------------------------------------------
    // Weight .mem files come from common/sv/cocotb/tests/weights/bitshift-custom_slow_decay/;
