// Nexys A7-100T board top for the fractional-16-4-32 SNN configuration.
...
//
// Pins (see constraints/nexys_a7_fractional_16_4_32.xdc):
...
// `cp_integration_frac-16-4-32` so the FPGA matches the simulated reference.

module board_top_fractional_16_4_32 (
    ...
    // --- Accelerator wrapper ------------------------------------------
    // Weight .mem files come from common/sv/cocotb/tests/weights/fractional-16-4-32/;
    // add that directory to the Vivado project's source search path so
    // $readmemh resolves the basenames below (including gl_coefficients.mem).
    wire        accel_busy;
    wire        accel_done;
    wire        accel_action;
