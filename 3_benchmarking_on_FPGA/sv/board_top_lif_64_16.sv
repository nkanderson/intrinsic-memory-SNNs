// Nexys A7-100T board top for the lif-64-16 SNN configuration.
...
//
// Pins (see constraints/nexys_a7_lif_64_16.xdc):
...
// `cp_integration_lif-64-16` so the FPGA matches the simulated reference.

module board_top_lif_64_16 (
    ...
    // (An earlier version OR'd in a fabric-sourced POR signal here, but
    // that became a high-fanout async-reset distribution path with ~37 ns
    // of routing delay, blowing recovery timing. CPU_RESETN comes off a
    // package pin and Vivado promotes it to a BUFG automatically.)
    wire reset_int;
    assign reset_int = ~CPU_RESETN;

    // --- Accelerator wrapper ------------------------------------------
    // Parameter values match the cocotb cp_integration_lif-64-16 target
    // and the model matrix in /home/stonepine/.claude/plans/i-need-to-start-federated-sutton.md.
    // Weight .mem files come from common/sv/cocotb/tests/weights/lif-64-16/;
