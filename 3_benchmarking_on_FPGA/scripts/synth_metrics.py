"""Parse Vivado synth/impl reports for one stage-3 config.

Expects the following canonical filenames under
3_benchmarking_on_FPGA/results/<config>/ (produced by build_config.tcl, or
copied manually from a GUI Vivado project):

  impl_utilization.rpt
  impl_timing_summary.rpt
  impl_power.rpt

Reports missing any of these are tolerated — the missing fields stay None.

Usage:
    python 3_benchmarking_on_FPGA/scripts/synth_metrics.py --config lif-64-16
    python 3_benchmarking_on_FPGA/scripts/synth_metrics.py            # all configs
"""
...
def parse_utilization(path: Path) -> dict[str, Optional[int]]:
    ...
    return {
        "slice_luts": _find_int_in_table(text, "Slice LUTs*") or _find_int_in_table(text, "Slice LUTs"),
...
def parse_timing(path: Path) -> dict[str, Optional[float]]:
    ...
    out: dict[str, Optional[float]] = {
        "wns_ns": None, "tns_ns": None, "whs_ns": None,
        "clock_period_ns": None, "fmax_est_mhz": None,
    }
    if not text:
        return out
...
def parse_power(path: Path) -> dict[str, object]:
    ...
    if not text:
        return {
            "power_total_w": None, "power_dynamic_w": None,
            "power_static_w": None, "junction_temp_c": None,
...
def parse_config(cfg: Config) -> dict[str, object]:
...
def main() -> int:
    ...
    parser.add_argument("--config", help="Single config name; default: all with synth artifacts")
    parser.add_argument("--csv", type=Path, help="Write results to CSV file")
    parser.add_argument("--json", action="store_true", help="Emit JSON to stdout instead of table")
    ...
    rows = [parse_config(c) for c in targets]

    if args.json:
        print(json.dumps(rows, indent=2, default=str))
    else:
        for r in rows:
            print(f"\n=== {r['config']} ({r['neuron_type']}) ===")
