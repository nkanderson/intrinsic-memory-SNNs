"""
Cocotb equivalence test for neural_network_bitshift: parallel vs. archived serial.

Drives the neural_network_bitshift_equivalence wrapper with identical
observations through both the new parallel `neural_network_bitshift` and the
archived `neural_network_bitshift_serial`, then asserts `selected_action`
matches exactly. Both networks share the same weight files and bitshift_lif
configuration, so identical inputs must produce identical actions if the
parallel rewrite preserves bit-exactness.
"""
...
TOTAL_BITS = 16
FRAC_BITS = 12  # bitshift variant uses QS3.12 weights
